 ...
 ?>       
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-6">
                         <p><a href="?tampil=tambah-potensi-sda" class="btn btn-primary btn-md" role="button">Tambah</a>
                         </p>
                    </div>
                    <div class="col-lg-6">
   ...
   if($cari) {
        // jika kotak pencarian tidak sama dengan kosong
            if($input_cari != "") {
                $sql =$mysqli->query("SELECT * FROM potensi where type='sda' AND
                judul like '%$input_cari%'
                ORDER BY judul ASC")or die($mysqli->error);
                } 
            else{
                    $sql =$mysqli->query("SELECT * FROM potensi where type='sda' ORDER BY judul ASC")or die($mysqli->error);
            }
        }
        else{
            $sql =$mysqli->query("SELECT * FROM potensi where type='sda' ORDER BY judul DESC")or die($mysqli->error);
        }
    $cek = $sql->num_rows;
   ...
   if($cek < 1) {
    ...
    ?>
             ...
             <a style="background-color: green;border-color: green; " href="?tampil=potensi-sda&id=<?php echo $data['id']; ?>" class="btn btn-danger btn-sm"> Lihat </a>
            <a style="background-color: #f84552 " href="?tampil=hapus-potensi-sda&id=<?php echo $data['id']; ?>" class="btn btn-danger btn-sm"> Hapus </a>
                                                </td>
                                            </tr>
                                           <?php 
                                           $no++;
